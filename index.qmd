...
<style>
/* "Zoom bigger" fragment: starts visible, scales up in place (stays centered)
   when its step is reached. Reveal's built-in `grow` only scales 1.3x. */
...
<!-- Order is 01 / 03 / 02 so the incorrect one (03) sits center and grows in place.
     Click 1: 01 & 02 gray (semi-fade-out), 03 enlarges (zoom-big). Click 2: the
     highlighted version fades in over 03. Click 3: the line appears. -->
...
::: notes
The problem isn't just that the model is sometimes wrong; it's that the wrong answers look exactly like the right ones. The confidence, the format, and the phrasing are identical. and you don't know its wrong until you look
:::
...
::: notes
callback to the opening "correctness matters" — reproducibility and transparency are also data-science values, all often at odds with LLMs.
:::
